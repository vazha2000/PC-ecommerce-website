...
import ComputersFiltersResp from './ComputersFiltersResp';
...
function ComputersList() {
  ...
  const filteredElements = [...elements.filter(item => item.price >= priceRange)]
  ...
  const prices = [...elements.map(item => item.price)]
  ...
  return (
    ...
    <div className='grid grid-cols-1 md:grid-cols-3 md:w-full '>
      ...
      {currentElements.filter(item => item.price >= priceRange).map(item =>   {
        return (
          <>
